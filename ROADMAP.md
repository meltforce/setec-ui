...
|---|---|---|---|---|
| [open] | **The app has no icon** | `Resources/Assets.xcassets/AppIcon.appiconset` | — the image provider has credit again | `icongen generate` refused on 2026-09-16 with HTTP 402, `Insufficient credits`, from the OpenRouter account the `mac-app-icon` skill draws through. The subject paragraph is written and is in the git history of this row: a private company vault for passwords and server keys, with a key on a labelled tag, a rack of numbered key tags, or a strongbox with a key on it. Nothing else blocks it; the appiconset still holds the scaffold's placeholder. |
