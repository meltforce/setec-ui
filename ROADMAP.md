...
| [open] | **A reuse scan goes stale without saying so** | `App/Features/Secrets/SecretStore.swift`, `App/Shell/SecretListView.swift` | | The digests describe the values as they were when the scan ran. A `put` from this app or from anywhere else makes an entry wrong, and the filter keeps showing the old answer. The scan carries its finish time but nothing shows it, and nothing invalidates the entry for a secret whose active version has changed since. Both are small; the question of whether a stale answer should be shown at all, greyed, or dropped is a design one. |
