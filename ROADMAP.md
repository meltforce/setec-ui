...
| Status | Item | Where | Trigger | Notes |
|---|---|---|---|---|
| [open] | **Per-version author and timestamp have no source** | `design/handoff.md` § *The setec data model*, the version table in the detail view | — decide before the version table is built | `list` returns version numbers only. Two ways out: drop the two columns, or carry the metadata elsewhere — a convention in the value envelope, or a sidecar store. A third, showing placeholder values, is excluded: a plausible-looking author who did not publish the version is worse than a missing column. The envelope option carries a cost that decides most of this: a wrapped value is no longer the value, so every existing consumer — every `setec get` in a `run.sh`, the hermes plugin, `setec.Store` in a Go service — would read JSON where it expects the secret. That rules the envelope out for existing names and leaves it plausible only for names this app creates, which is a split store. Undecided as of 2026-09-16. |
...
| [open] | **Where the server URL comes from is unsettled** | `Services/`, `App/Settings/SettingsView.swift` | — decide before the API client is written | Three candidates: a constant, the `SETEC_SERVER` environment variable the CLI reads, or a field in the Settings scene the scaffold already provides. The choice decides whether the store is constructed once at launch or has to survive a server change at runtime, and reworking that after the store exists is more expensive than picking now. It also interacts with the deferred multi-server row below. |
