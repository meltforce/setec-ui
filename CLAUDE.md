...
writing a view, `mac-app-verify` before claiming a task done. All three are
symlinks in `~/.claude/skills/` pointing into `mac-app-template`, and
`~/bin/axdump` and `~/bin/winid` are built from its `tools/`; both come from
`make install-tools install-skills` in that checkout, not from here. A Mac
without them still builds and runs the app: `make run` reports "no window
after 15 s" because the wait invokes `winid`, and `make screenshot` degrades to
the DebugServer's self-render, which leaves layer-hosted views such as the
sidebar blank.
